...
void Landscape::erode(int p_drops_per_frame){
	...
	for(int i=current_droplet; i < end_droplet; i++){
		...
		while(drop.water_amt > 0 && steps < max_steps && drop.pos.x >= 0 && drop.pos.x < map_w && drop.pos.y >= 0 && drop.pos.y < map_h){

			// find slope by neighbors
			...
			float slopeX = height_map[left] - height_map[right];
			float slopeZ = height_map[up] - height_map[down];
			...
			float max_capacity = std::max(speed * slope.length() * capacity_factor, min_capacity);
