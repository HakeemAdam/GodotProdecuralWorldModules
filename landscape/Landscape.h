...
private:
	...
	const float MESH_UPDATE_INTERVAL = 0.01f;
	int drops_per_frame;
...
public:
	...
	void set_capacity_factor(float p_rate);
	float get_capacity_factor();

	void set_min_capacity(float p_capacity);
	float get_min_capacity();

	void set_erosion_speed(float p_speed);
	float get_erosion_speed();

	void set_evaporation_rate(float p_rate);
	float get_evaporation_rate();
};
